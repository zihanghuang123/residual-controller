...
Loads TO trajectories from outputs/double_pendulum/trajectories.npz, samples rollout windows + plant parameters per iteration, and minimizes tracking + control regularization through differentiable MuJoCo. Saves params and loss history.
"""
...
def init_network(key):
    """Build the pure MLP controller and initialize params from a dummy input."""
    network = MLPPureController(hidden_sizes=cfg.PURE["hidden_sizes"], out_dim=cfg.NU)
    w = cfg.PURE["n_history"]
    nx = 2 * cfg.NQ
    in_dim = (w + 1) * nx + w * cfg.NU + (w + 1) * nx + (w + 1) * cfg.NU
...
def make_loss_fn(mjx_model_nominal, nominal_body_mass, network, x_refs, u_refs):
    ...
    def loss_fn(params, theta_key, idx, t0):
        ...
        def controller_fn(x_hist_full, u_hist, x_ref_window_w, u_ref_window_w):
            net_in = rollout.make_network_input(x_hist_full, u_hist, x_ref_window_w, u_ref_window_w)
            return network.apply(params, net_in)

        xs, us, vs = rollout.rollout(
            mjx_model, x_init, x_ref_window, u_ref_window,
            x_hist0, u_hist0, x_ref_hist0, u_ref_hist0,
            controller_fn, kp, kd, H,
        )

        return losses.tracking_loss(xs, x_ref_window, nq) + cfg.PURE["alpha_reg"] * losses.reg_loss(vs)
