#!/bin/bash
# Crocoddyl trajectory generation on blob — six_pendulum only (the longest).
set -uo pipefail
mkdir -p logs
...
echo "=== blob solve queue started: $(date) ==="
total_start=$(date +%s)

run_plant six

total=$(( $(date +%s) - total_start ))
echo "=== blob queue finished: $(date) (total $((total / 60))m$((total % 60))s) ==="
