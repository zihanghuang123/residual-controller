...
Same N_EVAL_PLANTS / EVAL_SEED protocol as evaluate.py, but restricted to the two controllers that don't need the theta estimator. Useful while iterating on the pure controller's window/horizon.
...
METRICS_PATH = OUTPUT_DIR / "metrics_pure.npz"
...
def make_eval_fn(mjx_model_nominal, nominal_body_mass, controller_fn, x_refs, u_refs, w):
    ...
    def eval_fn(theta_key, idx):
        ...
        x_ref_for_loss = x_refs[idx]      # (T+1, nx) — terminal-inclusive, matches evaluate.py
...
def main():
    ...
    print("loading pure params + building networks ...")
    ...
    np.savez(
        ...
        endpoint_pure=results["pure"][0], tracking_pure=results["pure"][1], vrms_pure=results["pure"][2],
    )
