...
def main():
    ...
    for ti in range(N_TRAJ_SHOW):
        ...
        for thi in range(N_THETA_SHOW):
            ...
            rollout = open_loop_factory(mjx_perturbed)

            u_residual = u_residual_labels[traj_idx, theta_idx]
            xs_noresid = np.asarray(rollout(jnp.asarray(x_init), jnp.asarray(u_nom)))
            xs_resid = np.asarray(rollout(jnp.asarray(x_init), jnp.asarray(u_nom + u_residual)))

            xs_noresid = np.vstack([x_init[None], xs_noresid])
            ...
            for j in range(nq):
                ...
                ax.plot(t, xs_noresid[:, j], "tab:red", lw=1, label="u_nom only")
                ax.plot(t, xs_resid[:, j], "tab:green", lw=1, label="u_nom + u_resid")
    ...
    err_noresid = np.zeros(N_TRAJ_SHOW * N_THETA_SHOW)
    err_resid = np.zeros(N_TRAJ_SHOW * N_THETA_SHOW)
    for ti in range(N_TRAJ_SHOW):
        x_init = x_refs[ti, 0]
        x_nom = x_refs[ti]
        u_nom = u_refs[ti]
        for thi in range(N_THETA_SHOW):
            ...
            rollout = open_loop_factory(mjx_perturbed)
            xs_noresid = np.asarray(rollout(jnp.asarray(x_init), jnp.asarray(u_nom)))
            xs_resid = np.asarray(rollout(jnp.asarray(x_init), jnp.asarray(u_nom + u_residual_labels[ti, thi])))
            idx = ti * N_THETA_SHOW + thi
            err_noresid[idx] = np.linalg.norm(xs_noresid[-1, :nq] - x_nom[-1, :nq])
            err_resid[idx] = np.linalg.norm(xs_resid[-1, :nq] - x_nom[-1, :nq])

    print(f"final qpos error vs x_nom:")
    print(f"  u_nom only:    mean={err_noresid.mean():.4f}  max={err_noresid.max():.4f}")
